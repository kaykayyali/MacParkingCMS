...
var handle_pay_input_submission = function() {
    ...
    if (name != undefined && gross_pay != "" && gross_pay > 0) {
		...
		$('#'+entries_count+'').prepend("<button class='btn btn-danger btn-xs delete_payroll_item'>x</button")
		$('#'+entries_count+'').on('click', function(event) {
			var id = $(event.currentTarget).attr('id');
			payload.payroll_data.forEach(function(item){
				if (item.id == id) {
					payload.payroll_data.splice(payload.payroll_data.indexOf(item), 1);
				} 
			});
